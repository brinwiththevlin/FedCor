...
class NaiveCNN(nn.Module, FedModule):
    def __init__(self, args, input_shape=[3, 32, 32], final_pool=True):
        ...
        if len(args.kernel_sizes) < len(args.num_filters):
            exlist = [
                args.kernel_sizes[-1]
                for i in range(len(args.num_filters) - len(args.kernel_sizes))
            ]
            args.kernel_sizes.extend(exlist)
        elif len(args.kernel_sizes) > len(args.num_filters):
            exlist = [
                args.num_filters[-1]
                for i in range(len(args.kernel_sizes) - len(args.num_filters))
            ]
        ...
        if not final_pool:
            if args.padding:
                ...
                output_shape[1:] = (
                    output_shape[1:] + 2 * pad - args.kernel_sizes[-1] + 1
                )
            else:
        ...
        self.convs.append(
            nn.Conv2d(
                ...
                padding=args.kernel_sizes[0] // 2 if args.padding else 0,
            )
        )
        for n in range(len(args.num_filters) - 1):
            self.convs.append(
                nn.Conv2d(
                    args.num_filters[n],
                    args.num_filters[n + 1],
                    kernel_size=args.kernel_sizes[n + 1],
                    padding=args.kernel_sizes[n + 1] // 2 if args.padding else 0,
                )
            )
...
class RNN(nn.Module):
    def __init__(
        self, n_hidden, n_classes, embedding=8, pretrain_emb=False, fc_layer=None
    ):
