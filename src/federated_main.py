...
if __name__ == "__main__":
    ...
    exp_details(args)
    if not args.iid:
        base_file = "./save/objects/{}_{}_{}_{}_C[{}]_iid[{}]_{}[{}]_E[{}]_B[{}]_mu[{}]_lr[{:.5f}]".format(
            args.dataset,
            "FedProx[%.3f]" % args.mu if args.FedProx else "FedAvg",
            args.model,
            args.epochs,
            args.frac,
            args.iid,
            "sp" if args.alpha is None else "alpha",
            args.shards_per_client if args.alpha is None else args.alpha,
            args.local_ep,
            args.local_bs,
            args.mu,
            args.lr,
        ...
        base_file = "./save/objects/{}_{}_{}_{}_C[{}]_iid[{}]_E[{}]_B[{}]_mu[{}]_lr[{:.5f}]".format(
            args.dataset,
            "FedProx[%.3f]" % args.mu if args.FedProx else "FedAvg",
            args.model,
            args.epochs,
    ...
    if args.afl:
        ...
        file_name = base_file + "/gpr[int{}_gp{}_norm{}_disc{}]".format(
            args.GPR_interval, args.group_size, args.poly_norm, args.discount
        )
    else:
    ...
    for seed in gargs.seed:
        ...
        train_dataset, test_dataset, user_groups, user_groups_test, weights = (
            get_dataset(args, seed)
        )
        ...
        local_states = (
            []
        )  # store local states of all users, these parameters should not be uploaded
        ...
        gt_global_losses = (
            []
        )  # test losses on global models(after averaging) over all clients
        ...
        bad_set = set(random.sample(range(args.num_users), args.num_users // 10))
        # Test the global model before training
        list_acc, list_loss = federated_test_idx(
            args,
            global_model,
        ...
        for epoch in tqdm(range(args.epochs)):
            ...
            if args.gpr and epoch > args.warmup:
                ...
                idxs_users = gpr.Select_Clients(
                    m, args.epsilon_greedy, weights, args.dynamic_C, args.dynamic_TH
                )
                ...
                # Power-of-D-choice
                A = np.random.choice(
                    range(args.num_users), args.d, replace=False, p=weights
                )
            ...
            for idx in idxs_users:
                ...
                w, test_loss, init_test_loss = local_update.update_weights(
                    model=local_model, mal=idx in bad_set
                )

                local_states[idx] = copy.deepcopy(local_model.Get_Local_State_Dict())
                local_weights[idx] = copy.deepcopy(w)
                epoch_global_losses.append(
                    init_test_loss
                )  # TAKE CARE: this is the test loss evaluated on the (t-1)-th global weights!
            ...
            if args.afl:
                AFL_Valuation[idxs_users] = np.array(epoch_global_losses) * np.sqrt(
                    weights[idxs_users] * len(train_dataset)
                )
            ...
            if args.gpr and epoch > args.warmup:
                ...
                test_data = np.concatenate(
                    [
                        np.expand_dims(list(range(args.num_users)), 1),
                        np.expand_dims(
                            np.array(gt_global_losses[-1])
                            - np.array(gt_global_losses[-2]),
                            1,
                        ),
                        np.ones([args.num_users, 1]),
                    ],
                    1,
                )
                ...
                try:
                    predict_loss, mu_p, sigma_p = gpr.Predict_Loss(
                        test_data, test_idx, pred_idx
                    )
                    print("GPR Predict relative Loss:{:.4f}".format(predict_loss))
                    predict_losses.append(predict_loss)
                except:
                    print(
                        "[Warning]: Singular posterior covariance encountered, skip the GPR test in this round!"
                    )
            ...
            if args.gpr:
                if epoch >= args.gpr_begin:
                    if epoch <= args.warmup:  # warm-up
                        gpr.Update_Training_Data(
                            [
                                np.arange(args.num_users),
                            ],
                            [
                                np.array(gt_global_losses[-1])
                                - np.array(gt_global_losses[-2]),
                        ...
                        epoch > args.warmup and epoch % args.GPR_interval == 0
                    ):  # normal and optimization round
                        gpr.Reset_Discount()
                        print("Training with Random Selection For GPR Training:")
                        random_idxs_users = np.random.choice(
                            range(args.num_users), m, replace=False
                        )
                        gpr_acc, gpr_loss = train_federated_learning(
                            args,
                            epoch,
                        ...
                        gpr.Update_Training_Data(
                            ...
                            [
                                np.array(gpr_loss) - np.array(gt_global_losses[-1]),
                            ],
                            epoch=epoch,
                        )
                        print("Training GPR")
        ...
        if args.gpr:
            print(
                "|---- Mean GP Prediction Loss: {:.4f}".format(np.mean(predict_losses))
            )
        ...
        with open(file_name + "_{}.pkl".format(seed), "wb") as f:
            pickle.dump(
                [
                    ...
                    chosen_clients,
                    weights,
                    None if not args.gpr else gpr.state_dict(),
                    gt_global_losses,
                    test_accuracy,
                ],
                f,
