...
def MVN_Sampler(
    ...
    for idxs in avg_idxs:
        ...
        _, updated_loss = federated_test_idx(
            args, avg_model, test_idxs, train_dataset, user_groups
        )
...
def MVN_Test(args, global_model, train_dataset, user_groups, file_name=None):
    ...
    _, init_loss = federated_test_idx(
        args, global_model, test_idxs, train_dataset, user_groups
    )
    ...
    with Manager() as manager:
        ...
        for i in range(args.mvnt_workers):
            avg_idxs = [
                ...
                for _ in range(sample_per_worker)
            ]
            p = Process(
                target=MVN_Sampler,
                args=(
