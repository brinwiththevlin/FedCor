    ...
    if args.dataset == "cifar":
        ...
        if args.iid:
            ...
            if args.alpha is not None:
                user_groups, _ = Dirichlet_noniid(
                    train_dataset, args.num_users, args.alpha, rs
                )
                user_groups_test, _ = Dirichlet_noniid(
                    test_dataset, args.num_users, args.alpha, rs
                )
            elif args.unequal:
                ...
                user_groups = cifar_noniid(
                    train_dataset, args.num_users, args.shards_per_client, rs
                )
                user_groups_test = cifar_noniid(
                    test_dataset, args.num_users, args.shards_per_client, rs
                )
        ...
        if args.iid:
            ...
            if args.alpha is not None:
                ...
                user_groups_test = mnist_noniid_unequal(
                    test_dataset, args.num_users, rs
                )
            else:
                user_groups = mnist_noniid(
                    train_dataset, args.num_users, args.shards_per_client, rs
                )
                user_groups_test = mnist_noniid(
                    test_dataset, args.num_users, args.shards_per_client, rs
                )
        ...
        user_groups_test = {}
        train_dataset, test_dataset, user_groups = shakespeare(
            data_dir, args.shards_per_client, rs
        )
    elif args.dataset == "sent":
        args.num_classes = 2
        data_dir = "./data/sent140/"
        user_groups_test = {}
        train_dataset, test_dataset, user_groups = sent140(
            data_dir, args.shards_per_client, rs
        )
...
if __name__ == "__main__":
    ...
    print(args.dataset)
    train_dataset, test_dataset, user_groups, user_groups_test, weights = get_dataset(
        args
    )
