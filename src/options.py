...
def args_parser():
    ...
    parser.add_argument(
        "--num_users", type=int, default=100, help="total number of clients: N"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--reg", default=1e-4, type=float, help="weight decay for an optimizer"
    )
    parser.add_argument(
        "--global_average",
        action="store_true",
    ...
    parser.add_argument("--FedProx", action="store_true", help="use FedProx")
    parser.add_argument("--mu", type=float, default=0.0, help="mu in FedProx")
    parser.add_argument(
        "--dynamic_mu", action="store_true", help="use a dynamic mu for FedProx"
    )
    ...
    parser.add_argument(
        "--warmup", type=int, default=25, help="length of warm up phase for GP"
    )
    parser.add_argument(
        "--gpr_begin",
        type=int,
    ...
    parser.add_argument(
        "--GPR_gamma", type=float, default=0.8, help="gamma for training GP"
    )
    parser.add_argument(
        "--GPR_Epoch",
        type=int,
    ...
    parser.add_argument(
        "--kernel", type=str, default="Poly", help="kind of kernel used in GP (Poly,SE)"
    )
    parser.add_argument(
        "--poly_norm",
        type=int,
    ...
    parser.add_argument(
        "--dimension", type=int, default=15, help="dimension of embedding in GP"
    )
    parser.add_argument(
        "--train_method",
        type=str,
    ...
    parser.add_argument(
        "--dynamic_C", action="store_true", help="use dynamic GP clients selection"
    )
    parser.add_argument(
        "--dynamic_TH", type=float, default=0.0, help="dynamic selection threshold"
    )
    parser.add_argument(
        "--mvnt", action="store_true", help="Perform Multivariate Normality Test"
    )
    parser.add_argument(
        "--mvn_samples",
        type=int,
        default=1000,
        help="Number of samples for each MVN Test",
    )
    parser.add_argument(
        "--mvn_dimensions",
        type=int,
        default=100,
        help="Number of dimensions (clients) for MVN Test",
    )
    parser.add_argument(
        "--mvnt_interval", type=int, default=100, help="Interval for MVN Test"
    )
    parser.add_argument(
        "--mvnt_workers", type=int, default=8, help="Number of workers for MVN sampling"
    )

    # Power-d arguments
    parser.add_argument("--power_d", action="store_true", help="use Pow-d selection")
    parser.add_argument("--d", type=int, default=30, help="d in Pow-d selection")

    # Active Federated Learning arguments
    parser.add_argument("--afl", action="store_true", help="use AFL selection")
    parser.add_argument("--alpha1", type=float, default=0.75, help="alpha_1 in ALF")
    parser.add_argument("--alpha2", type=float, default=0.01, help="alpha_2 in AFL")
    parser.add_argument("--alpha3", type=float, default=0.1, help="alpha_3 in AFL")
    ...
    parser.add_argument(
        "--padding", action="store_true", help="use padding in each convolutional layer"
    )
    parser.add_argument(
        "--mlp_layers",
        type=int,
    ...
    parser.add_argument("--seed", type=int, default=None, nargs="*", help="random seed")
    parser.add_argument(
        "--target_accuracy",
        type=float,
        default=None,
