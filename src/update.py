...
class DatasetSplit(Dataset):
    ...
    def __init__(self, dataset, idxs, mal, lt):
        self.dataset = dataset
        self.idxs = [int(i) for i in idxs]
        if mal:
            self.label_tampering(lt)
...
class LocalUpdate(object):
    def __init__(self, args, dataset, idxs, global_round=0, verbose=None, mal=False):
        self.args = args
        # self.logger = logger
        self.trainloader, self.testloader = self.train_test(dataset, list(idxs), mal)
    ...
    def train_test(self, dataset, idxs, mal):
        """
        ...
        trainloader = DataLoader(
            DatasetSplit(dataset, idxs_train, mal, self.args.label_tampering),
            batch_size=self.args.local_bs,
            shuffle=True,
        )
        testloader = DataLoader(
            DatasetSplit(dataset, idxs_test, mal, self.args.label_tampering),
            batch_size=max([int(len(idxs_test) / 10), 10]),
            shuffle=False,
        )
        return trainloader, testloader

    def update_weights(self, model, mal=None):
        ...
        if self.args.optimizer == "sgd":
            ...
            optimizer = torch.optim.Adam(
                np, lr=self.args.lr, weight_decay=self.args.reg
            )
        ...
        for iter in range(self.args.local_ep):
            ...
            if self.verbose:
                print(
                    "| Global Round : {} | Local Epoch : {} |\tLoss: {:.4f}\tCE_Loss: {:.4f}\tProxy_Loss: {:.4f}".format(
                        self.global_round,
                        iter,
                        total_loss.item(),
                        celoss,
                        PLoss if self.args.FedProx else 0.0,
                    )
                )
...
def train_federated_learning(
    args, epoch, global_model, idxs_users, train_dataset, user_groups, verbose=False
):
    ...
    for idx in range(args.num_users):
        local_model = copy.deepcopy(global_model)
        local_update = LocalUpdate(
            args=args, dataset=train_dataset, idxs=user_groups[idx], verbose=verbose
        )
...
def federated_train_all(args, global_model, train_dataset, user_groups):
    ...
    for idx in tqdm(range(args.num_users)):
        local_model = copy.deepcopy(global_model)
        local_update = LocalUpdate(
            args=args, dataset=train_dataset, idxs=user_groups[idx], verbose=False
        )
...
def federated_test_idx(
    args, global_model, idxs_users, train_dataset, user_groups, bad_set
):
    device = "cuda:" + args.gpu if args.gpu else "cpu"
    global_model.eval()
    list_acc, list_loss = [], []
    ...
    for idx in idxs_users:
        mal = idx in bad_set
        local_update = LocalUpdate(
            args=args, dataset=train_dataset, mal=mal, idxs=user_groups[idx]
        )
        acc, loss = local_update.inference(model=local_model)
        list_acc.append(acc)
        list_loss.append(loss)
