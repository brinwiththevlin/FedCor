...
The official implementation of the Paper: _FedCor: Correlation-Based Active Client Selection Strategy for Heterogeneous Federated Learning_ (CVPR 22').
...
```shell
python3 src/federated_main.py  --dataset=mnist --model=mlp --mlp_layer 64 30  --epochs=100 --num_user=100 --frac=0.05 --alpha=0.2 --local_ep=3 --local_bs=64 --lr=5e-3 --schedule 150 300 --lr_decay=0.5 --optimizer=sgd --iid=0 --unequal=0  --verbose=1 --seed 8451 --gpr --poly_norm=0 --GPR_interval=10 --group_size=100 --GPR_gamma=0.95 --update_mean --warmup=15 --discount=0.95
```
